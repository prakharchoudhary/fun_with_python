"""
fork child processes to watch exit status with os.wait; fork work on Unix
and Cygwin but not standard Windows Python 3.1; note: spawned threads share 
globals, but each forked process has its own copy of them (forks share file descriptors)
...
def child():					# could os.exit a script here
	...
	exitstat += 1				# exit status to parent's will
